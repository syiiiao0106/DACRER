...
class BertDNNForRoleLabeling(BertForTokenClassification):
    def __init__(self, config, use_cls=False, prod_pooler="head"):
        super(BertDNNForRoleLabeling, self).__init__(config)
    ...
    def forward(
        ...
        # dataA = sequence_output.data.cpu().numpy()
        # scio.savemat('PredictSegmentEmbedding.mat', {'sequence_output': dataA}) # SegmentEmbedding
        ...
        # dataB = extended_prod_h.data.cpu().numpy()
        # scio.savemat('PredictPositionEmbedding.mat', mdict={'extended_prod_h': dataB})
        ...
        if self.use_cls:
            ...
            # dataC = extended_cls_h.data.cpu().numpy()
            # scio.savemat('PredictTokenEmbedding.mat', mdict={'extended_cls_h': dataC})
        ...
        final_out = self.classifier(extended_sequence_output) 
        temp1 = self.linear1(final_out) 
...
class BertDNNForTagging(BertForTokenClassification):
    def __init__(self, config, use_cls=False):
        super(BertDNNForTagging, self).__init__(config)
    ...
    def forward(
        ...
        final_out = self.classifier(sequence_output) 
        temp1 = self.linear1(final_out)
        logits = self.linear2(temp1)
