                ...
                if line.startswith("#\tpassage"):
                    ...
                    if words:
                        ...
                        for etype, ss, se in prods:
                            ...
                            # if etype == "Reactants":
                            #     inst_labels = ["O"] * len(words)
                            #     inst_labels[ss] = "B-Reactants"
                            #     inst_labels[ss+1:se+1] = ["I-Reactants"] * (se-ss)
                            # el
                            if etype == "Prod":# a 'keng' or not ?????
                                inst_labels = ["O"] * len(words)
                                inst_labels[ss] = "B-Prod"
                                inst_labels[ss+1:se+1] = ["I-Prod"] * (se-ss)
                            # elif etype == "Yield":
                            #     inst_labels = ["O"] * len(words)
                            #     inst_labels[ss] = "B-Yield"
                            #     inst_labels[ss+1:se+1] = ["I-Yield"] * (se-ss)
                                examples.append(
                                    InputExample(
                                        guid=f"{guid_index}",
                                        words=words,
...
def write_predictions(input_file, output_file, predictions, align="labeled"):
    ...
    if align == "labeled":
        with open(output_file, "w",encoding='utf-8') as writer, open(input_file, "r",encoding='utf-8') as f:
            example_id = 0
            num_rxns = 0
            for line in tqdm(f):
                line = line.rstrip()
                if line.startswith("#\tpassage"):
                    ...
                    for j, label in enumerate(cols[1:]):
                        if((example_id+j)==len(predictions)):
                            break
                        output_line.append(label)
                        if label in ["B-Prod", "I-Prod"]:
                            output_line.append(label)
                        else:
                            # print("example id+j is %d\n" %(example_id+j))
                            # print("len(predictions) is %d\n" %(len(predictions)))
